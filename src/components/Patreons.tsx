...
export default function Patreons() {
    return (
        <section id="patreons" className="w-full bg-gray-500 dark:bg-gray-800 md:min-h-64 sm:h-80">
                    ...
                    <div className="carrusel flex gap-2 ">
                        {
                            sponsors.map((sponsor) => (
                                <Patreon nombre={sponsor.nombre} link={sponsor.url} alt="Lucas" img={sponsor.img} />
                            ))
                        }

                    </div>
                </div>
                <h3 className="text-gray-800">Información requerida: sponsors y demas</h3>
            </div>
        </section>
    )
}
